...
class pjAdminCategories extends pjAdmin
{
	...
	public function pjActionDeleteCategory()
	{
		...
		if ($this->isXHR())
		{
			...
			pjQuestionCategoryModel::factory()->where('category_id', $_GET['id'])->eraseAll();
			$response['code'] = 200;
	...
	public function pjActionDeleteCategoryBulk()
	{
		...
		if ($this->isXHR())
		{
			if (isset($_POST['record']) && count($_POST['record']) > 0)
			{
				...
				pjQuestionCategoryModel::factory()->whereIn('category_id', $_POST['record'])->eraseAll();
			}
		}
		exit;
	}
	...
	public function pjActionGetCategory()
	{
		...
		if ($this->isXHR())
		{
			...
			$c_arr = $pjCategoryModel
				->reset()
				->select(sprintf("t1.id, (SELECT COUNT(*) FROM `%s` WHERE `category_id` = `t1`.`id` LIMIT 1) AS `questions`", pjQuestionCategoryModel::factory()->getTable()))
				->findAll()
				->getDataPair('id', 'questions');
			...
			{
				$data[$k]['questions'] = (int) @$c_arr[$category['data']['id']];
				$data[$k]['up'] = 0;
				$data[$k]['down'] = 0;
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin())
		{
			...
			$this->set('has_up_down', $has_up_down);
